...
const storageHandlers = {
  ...
  async getBulk(keys) {
    ...
    for (const key of keys) {
      results[key] = await storageHandlers.get(key);
    }
    return results;
  },
  ...
  async setBulk(data) {
    for (const [key, value] of Object.entries(data)) {
      await storageHandlers.set(key, value);
    }
    return true;
  }
};

contextBridge.exposeInMainWorld('storageAPI', storageHandlers);
